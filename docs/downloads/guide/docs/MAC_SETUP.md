...
2. 最新版の未使用ZIPを展開し、AI-NORIKO-Starter-KitフォルダをCodexで開きます。START_HERE.mdが見えることを確認します。既存環境がある場合は別の場所へ展開し、名前・場所は初回起動前に決めます。
...
設定の「このAI NORIKOの保存先」で環境ID・本体フォルダ・保存先を確認します。標準ナレッジは `Documents/AI-NORIKO-Instances/<環境ID>/Knowledge` です。別フォルダで導入すれば別環境ですが、同じObsidianやGoogleを選ぶと外部のデータは共有されます。
...
再開は同じ本体フォルダから行います。本体の移動・名前変更で初期画面になった場合、元データを削除せず [別環境の導入](MULTIPLE_INSTALLS.md) を確認します。旧v1.1の共有設定は自動移行しません。
